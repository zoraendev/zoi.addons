...
class AdvancedMetricsApiConfig(models.Model):
    ...
    access_token = fields.Char(string='Token de Acceso', copy=False, default=lambda self: str(uuid.uuid4()), readonly=True)
    record_limit = fields.Integer(string='Limite de Registros', default=5000, required=True, help='Previene ataques de denegacion de servicio limitando la extraccion maxima.')
    api_url = fields.Char(string='URL para Power BI', compute='_compute_api_url')
