...
class AdvancedMetricsReportWizard(models.TransientModel):
    ...
    def _normalize_sales_trend_filters(self, filters=None):
        ...
        normalized_filters = {
            ...
            'dateTo': None,
            'warehouseId': self._parse_optional_integer(filters.get('warehouseId'), 'warehouseId'),
            'groupBy': (filters.get('groupBy') or 'day').strip().lower(),
        }
    ...
    def _normalize_dead_products_filters(self, filters=None):
        filters = filters or {}
        raw_days_without_movement = (
            filters.get('daysWithoutMovement')
            or filters.get('days_without_movement')
            or 60
        )

        try:
            days_without_movement = int(raw_days_without_movement)
        except (TypeError, ValueError):
            raise ValidationError('El filtro "daysWithoutMovement" debe ser un numero entero.')

        if days_without_movement < 0:
            raise ValidationError('El filtro "daysWithoutMovement" no puede ser negativo.')
    ...
    def _get_last_sale_date_by_product(self, product_ids, warehouse_id=None):
        ...
        last_sale_dates = {}
        lines = sale_line_model.search(domain, order='id desc')
    ...
    def get_products_sales_report_data(self, filters=None):
        ...
        if normalized_filters['dateTo']:
            domain.append(('order_id.date_order', '<=', f"{normalized_filters['dateTo']} 23:59:59"))
        if normalized_filters['warehouseId']:
            domain.append(('order_id.warehouse_id', '=', normalized_filters['warehouseId']))
        if normalized_filters['categoryId']:
            domain.append(('product_id.categ_id', 'child_of', normalized_filters['categoryId']))
        ...
        products = product_model.browse(product_ids).exists()
    ...
    def get_sales_trend_report_data(self, filters=None):
        ...
        anchor_date = fields.Date.to_date(normalized_filters['dateTo']) if normalized_filters['dateTo'] else fields.Date.to_date(fields.Date.context_today(self))
