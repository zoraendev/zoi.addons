...
class PbiConnectionsInventoryDashboard(models.AbstractModel):
    _name = 'pbi_connections.inventory.dashboard'
    _description = 'Servicio del dashboard de inventario para PBI Connections'
    ...
    def get_weekly_production_plan_report_data(self, filters=None):
        ...
        rows = self._get_weekly_plan_wizard_model().sudo().get_sales_orders_report_rows(wizard_filters)
        return {
            'generatedAt': self._get_generated_at_iso(),
