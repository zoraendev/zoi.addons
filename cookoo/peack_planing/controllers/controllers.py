...
class PeackPlaningController(http.Controller):
    """
    ...
    @http.route('/peack_planing/report/next-week-dates', type='json', auth='user', methods=['POST'], csrf=False)
    def get_next_week_dates(self, **kwargs):
        ...
        """
        return request.env['peack_planing.report.wizard'].sudo().get_next_week_dates()

    @http.route('/peack_planing/report/period-dates', type='json', auth='user', methods=['POST'], csrf=False)
    def get_period_dates(self, **kwargs):
        ...
        return request.env['peack_planing.report.wizard'].sudo().get_period_dates(period_type)

    @http.route('/peack_planing/report/generate', type='http', auth='user', methods=['POST'], csrf=False)
    def generate_sales_orders_report(self, **kwargs):
        ...
        rows = request.env['peack_planing.report.wizard'].sudo().get_sales_orders_report_rows(filters)
    ...
    def _create_xlsx_styles(self, workbook):
        ...
        brand_color = '#7c6a8b'  # Color PeackPlaning
        return {
            'header': workbook.add_format({'bold': True, 'bg_color': brand_color, 'font_color': 'white', 'border': 1, 'align': 'center', 'valign': 'vcenter'}),
            'text': workbook.add_format({'border': 1, 'valign': 'vcenter'}),
            'text_alt': workbook.add_format({'border': 1, 'bg_color': '#F8F9FA', 'valign': 'vcenter'}),
            'number': workbook.add_format({'border': 1, 'num_format': '#,##0.00', 'valign': 'vcenter'}),
            'number_alt': workbook.add_format({'border': 1, 'num_format': '#,##0.00', 'bg_color': '#F8F9FA', 'valign': 'vcenter'}),
            'subtotal_text': workbook.add_format({'bold': True, 'bg_color': '#E9ECEF', 'border': 1, 'font_color': brand_color, 'valign': 'vcenter'}),
            'subtotal_number': workbook.add_format({'bold': True, 'bg_color': '#E9ECEF', 'border': 1, 'num_format': '#,##0.00', 'font_color': brand_color, 'valign': 'vcenter'}),
            'total_label': workbook.add_format({'bold': True, 'bg_color': brand_color, 'font_color': 'white', 'border': 1, 'valign': 'vcenter'}),
        }
