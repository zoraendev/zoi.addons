...
class PeackPlaningReportWizard(models.TransientModel):
    _name = 'peack_planing.report.wizard'
    ...
    def _sync_selected_review_lines(self, candidate_lines=None):
        ClientLine = self.env['peack_planing.report.wizard.client.line']
        ProductLine = self.env['peack_planing.report.wizard.product.line']
    ...
    def action_clear_filters(self):
        ...
        self._reset_report_payload()
        self._compute_review_data()
        filter_view = self.env.ref('peack_planing.view_peack_planing_report_wizard_form')
        return {
            'type': 'ir.actions.act_window',
            'name': 'Ordenes de venta',
            'res_model': 'peack_planing.report.wizard',
            'res_id': self.id,
            'view_mode': 'form',
    ...
    def _load_report_payload(self, rows):
        Line = self.env['peack_planing.report.wizard.line']
        for wizard in self:
            wizard.report_line_ids.unlink()
            line_values = []
            for row in rows:
    ...
    def action_generate_report(self):
        ...
        report_view = self.env.ref('peack_planing.view_peack_planing_report_wizard_report_form')
        return {
            'type': 'ir.actions.act_window',
    ...
    def action_open_production_summary(self):
        self.ensure_one()
        production_view = self.env.ref('peack_planing.view_peack_planing_report_wizard_production_form')
        return {
            'type': 'ir.actions.act_window',
...
class PeackPlaningReportWizardClientLine(models.TransientModel):
    _name = 'peack_planing.report.wizard.client.line'
...
class PeackPlaningReportWizardProductLine(models.TransientModel):
    _name = 'peack_planing.report.wizard.product.line'
